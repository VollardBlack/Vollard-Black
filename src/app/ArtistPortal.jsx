...
const C = { gold:'#b68b2e',goldD:'#8a6a1e',goldL:'rgba(182,139,46,0.12)',goldB:'rgba(182,139,46,0.22)',cream:'#f5f3ef',dark:'#1a1714',mid:'#6b635a',light:'#8a8070',red:'#c45c4a',green:'#4a9e6b',greenD:'#2d7a4a',blue:'#648cc8',white:'#ffffff',smoke:'#f0ede8' ,white:'#ffffff',smoke:'#f0ede8'};
...
function LoginScreen({onLogin,onRegister,role,portalLabel}){
  ...
  const handleSignUp=async()=>{
    ...
    await supabase.from('portal_requests').upsert({id:crypto.randomUUID(),email:email.trim().toLowerCase(),role:'artist',status:'pending',created_at:new Date().toISOString()},{onConflict:'email,role'}).catch(()=>{});
    setLoading(false);
