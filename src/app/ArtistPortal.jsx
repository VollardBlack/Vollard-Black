...
export default function ArtistPortal(){
  ...
  const[status,setStatus]=useState('init'); // init|checking|approved|pending|none
  ...
  // Check portal_requests when session changes
  useEffect(()=>{
    if(session===undefined)return;
    if(!session){setStatus('none');setTermsSigned(false);return;}
    setStatus('checking');
    const timer=setTimeout(()=>setStatus('approved'),5000); // fallback: let through after 5s
    sb.from('portal_requests')
      ...
      .then(({data})=>{
        clearTimeout(timer);
        if(!data){
          // No row = admin-created user or just signed up — let them through, admin will see them
          setStatus('approved');
      ...
      .catch(()=>{clearTimeout(timer);setStatus('approved');}); // on error, let through
  },[session]);

  // Check terms when approved
  useEffect(()=>{
    if(status!=='approved'||!session)return;
    // Fast localStorage check first
    try{
      const s=JSON.parse(localStorage.getItem('vb_terms_artist')||'null');
      if(s&&s.email===session.user.email&&s.v===TERMS_VERSION){setTermsSigned(true);return;}
    }catch{}
  ...
  if(!session||status==='none')
    return<AuthScreen onAuth={(sess,type)=>{setSession(sess);setPendingEmail(sess?.user?.email||'');}} />;

  // Pending approval
  if(status==='pending')
    return<NotApprovedScreen onSignOut={()=>sb.auth.signOut()}/>;

  // Show terms if not signed
  if(!termsSigned)
    return<TermsModal email={session.user.email} onAccepted={()=>setTermsSigned(true)}/>;

  // Approved + signed — show dashboard
