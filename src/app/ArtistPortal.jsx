      ...
      <div style={{maxWidth:960,margin:'0 auto',padding:'24px 16px 100px'}}>
        ...
        {tab==='overview'&&(
          <div>
            ...
            {sales.length>0&&<div style={CARD2}><div style={{...CP}}><div style={SH}>Price History</div>{sales.map((s,i)=>{const art=artworks.find(a=>a.id===s.artworkId);const reserve=art?.recommendedPrice||0;const sold=s.salePrice||0;const pct=reserve>0?Math.round((sold/reserve)*100):100;return(<div key={i} style={{marginBottom:14}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}><span style={{fontSize:13,color:textPrimary,fontWeight:500}}>{s.artworkTitle}</span><span style={{fontSize:13,color:pct>=100?C.green:C.gold,fontWeight:700}}>R {fmt(sold)} <span style={{fontSize:10,opacity:0.7}}>({pct}% of value)</span></span></div><div style={{height:6,background:darkMode?'rgba(255,255,255,0.1)':'rgba(182,139,46,0.10)',borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',width:`${Math.min(pct,150)}%`,background:pct>=100?`linear-gradient(90deg,${C.green},#2d9e5a)`:`linear-gradient(90deg,${C.gold},${C.goldD})`,borderRadius:3,transition:'width 1s ease'}}/></div></div>);})}</div></div></div>}
