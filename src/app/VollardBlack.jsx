...
const MODELS = {
  A24: { vbPct: 0.40, colPct: 0.60, term: 24, label: "Model A · 24mo", short: "A24" },
  A36: { vbPct: 0.40, colPct: 0.60, term: 36, label: "Model A · 36mo", short: "A36", minValue: 100001 },
...
const generateSettlementPDF=(sale,artworkValue,monthsPaid,acquisitionModel,galleryPct,vbPct,artistPct,introFeePct)=>{
  const m=MODELS[acquisitionModel||"A24"];
  const deal=calcDeal(artworkValue,sale.salePrice,acquisitionModel||"A24",monthsPaid,galleryPct||40,vbPct||30,artistPct||30,introFeePct||0);
...
<div class="artwork-strip">
  <div>
    ...
    <div class="people">Collector: <strong>${sale.collectorName||"—"}</strong>&nbsp;&nbsp;·&nbsp;&nbsp;Buyer: <strong>${sale.buyerName||"—"}</strong></div>
  </div>
...
<div class="two-col">
  <div class="section">
    ...
    <table>
      ...
      <tr><td class="lbl">VB fee (${Math.round(m.vbPct*100)}%)</td><td class="gold">R ${fmt(deal.vbFee)}</td></tr>
...
<div class="section">
  <div class="section-title">Backend Split — VB's ${Math.round(m.vbPct*100)}% Fee</div>
...
const modelC={A24:{bg:"rgba(74,158,107,0.12)",c:"#4a9e6b",label:"Model A · 24mo"},A36:{bg:"rgba(100,140,200,0.12)",c:"#648cc8",label:"Model A · 36mo"},B:{bg:"rgba(182,139,46,0.12)",c:"#b68b2e",label:"Model B · 12mo"}};
...
export default function App(){
  ...
  const actions={
    ...
    recordPayment:(schedule,monthNumber,method,amount)=>{
      const payment={id:uid(),scheduleId:schedule.id,collectorId:schedule.collectorId,collectorName:schedule.collectorName,artworkId:schedule.artworkId,artworkTitle:schedule.artworkTitle,model:schedule.acquisitionModel||"A24",monthNumber,amount,method,date:td(),createdAt:td()};
    ...
    <div style={{display:"flex",minHeight:"100vh",background:"#0c0b09",fontFamily:"DM Sans,sans-serif",color:"#e8e2d6"}}>
      ...
      <aside style={{width:240,minHeight:"100vh",background:"#111010",borderRight:"1px solid rgba(182,139,46,0.1)",display:"flex",flexDirection:"column",position:"fixed",left:sb?0:"-240px",top:0,bottom:0,zIndex:99,transition:"left 0.3s",...(typeof window!=="undefined"&&window.innerWidth>900?{position:"relative",left:0}:{})}}>
        ...
        <div style={{padding:"16px 24px",borderTop:"1px solid rgba(182,139,46,0.08)",fontSize:10,color:"#5a564e",letterSpacing:2}}>
          <div>MODEL A: 40/60 · 24 OR 36 MO</div>
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      <Card><div style={{fontSize:14,fontWeight:600,color:"#f5f0e8",marginBottom:16}}>Active Schedules</div>
        {upcomingSchedules.length===0?<p style={{fontSize:13,color:"#5a564e"}}>No active schedules.</p>:upcomingSchedules.map(s=>{const pct=s.termMonths>0?(s.monthsPaid/s.termMonths)*100:0;return<div key={s.id} style={{padding:"10px 0",borderBottom:"1px solid rgba(182,139,46,0.04)"}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,alignItems:"center"}}><span style={{fontWeight:500}}>{s.collectorName}</span><div style={{display:"flex",alignItems:"center",gap:8}}><Badge model={s.acquisitionModel||"A24"}/><span style={{color:"#b68b2e",fontWeight:600}}>R {fmt(s.monthlyAmount)}/mo</span></div></div><div style={{fontSize:11,color:"#5a564e",marginBottom:4}}>{s.artworkTitle} · Month {s.monthsPaid} of {s.termMonths}</div><ProgressBar pct={pct}/></div>;})}
      </Card>
...
function LinkMdl({col,arts,onLink,onClose,gn}){
  ...
  const [acqModel,setAcqModel]=useState("A24");
  ...
  return(<Modal title={`Link Artwork — ${gn(col)}`} onClose={onClose} wide>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
      {["A24","A36","B"].map(k=>{const mod=MODELS[k];const active=acqModel===k;const locked=k==="A36"&&art&&art.recommendedPrice<=100000;return<button key={k} onClick={()=>!locked&&setAcqModel(k)} style={{padding:12,borderRadius:10,border:active?"2px solid #b68b2e":locked?"1px solid rgba(182,139,46,0.06)":"1px solid rgba(182,139,46,0.15)",background:active?"rgba(182,139,46,0.08)":locked?"rgba(0,0,0,0.2)":"#1e1d1a",color:active?"#b68b2e":locked?"#3a3832":"#8a8477",cursor:locked?"not-allowed":"pointer",fontFamily:"DM Sans,sans-serif",textAlign:"left",transition:"all 0.15s",opacity:locked?0.5:1}}>
        ...
        {k==="A36"&&<div style={{fontSize:10,color:locked?"#3a3832":"#5a564e",marginTop:2}}>R 100k+ only</div>}
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",gap:12,marginTop:4,fontSize:12}}>
              ...
              <Badge model={sale.acquisitionModel||"A24"}/>
            </div>
          </div>
        </div>
      </div>;
    ...
    <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:24,alignItems:"start"}}>
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
          {["A24","A36","B"].map(k=>{const mod=MODELS[k];const on=acqModel===k;return<button key={k} onClick={()=>{setAcqModel(k);setMonthsSold("");}} style={{padding:12,borderRadius:10,border:on?"2px solid #b68b2e":"1px solid rgba(182,139,46,0.15)",background:on?"rgba(182,139,46,0.08)":"#1e1d1a",color:on?"#b68b2e":"#8a8477",cursor:"pointer",fontFamily:"DM Sans,sans-serif",textAlign:"left"}}>
...
function InvoicePage({data,actions,initialFilter,clearFilter}){
  ...
  return(<div>
    ...
    pagedSchedules.map(sched=>{
      const m=MODELS[sched.acquisitionModel||"A24"];
          ...
          <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:4}}>
                ...
                <Badge model={sched.acquisitionModel||"A24"}/>
    ...
    {payModal&&<Modal title={`Record Payment — Month ${payModal.nextMonth}`} onClose={()=>setPayModal(null)}>
      <Card style={{background:"#1e1d1a",marginBottom:16}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:13}}>
        ...
        <span style={{color:"#8a8477"}}>Model:</span><span><Badge model={payModal.sched.acquisitionModel||"A24"}/></span>
...
function SalesPage({data,actions}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:14,marginBottom:24}}>
      ...
      <Stat label="Model A Sales" value={data.sales.filter(s=>!s.acquisitionModel||["A","A24","A36"].includes(s.acquisitionModel)).length} green/>
    ...
    <Card>
      ...
      <Tbl cols={[
        ...
        {label:"Model",render:r=><Badge model={r.acquisitionModel||"A24"}/>},
...
function SettlementModal({sale,data,onClose}){
  ...
  const acqModel=sched?.acquisitionModel||sale.acquisitionModel||"A24";
...
function SaleMdl({data,sellable,onSale,onClose}){
  ...
  const gn=c=>c?(c.type==="company"?c.companyName:`${c.firstName} ${c.lastName}`):"—";
  const acqModel=sched?.acquisitionModel||"A24";
  const m=MODELS[acqModel];
  ...
  return(<Modal title="Record Sale" onClose={onClose} wide>
    <Field label="Artwork"><select value={artId} onChange={e=>setArtId(e.target.value)} style={ss}><option value="">—</option>{sellable.map(a=>{const s=data.schedules.find(x=>x.artworkId===a.id);return<option key={a.id} value={a.id}>{a.title} — R {fmt(a.recommendedPrice)} {s?`(${MODELS[s.acquisitionModel||"A24"].label})`:""}</option>;})} </select></Field>
        ...
        <div style={{padding:20}}>
          ...
          {report&&<>
            ...
            {selectedMonth===ym&&<div style={{marginTop:10,borderTop:"1px solid rgba(182,139,46,0.06)",paddingTop:10}}>
              {report.snapshot.payments&&report.snapshot.payments.length>0?<Tbl cols={[{label:"Collector",bold:true,render:r=>r.collectorName},{label:"Artwork",key:"artworkTitle"},{label:"Model",render:r=><Badge model={r.model||"A24"}/>},{label:"Month",render:r=>`Mo ${r.monthNumber}`},{label:"Method",key:"method"},{label:"Amount",right:true,gold:true,render:r=>"R "+fmt(r.amount)}]} data={report.snapshot.payments}/>:<p style={{fontSize:13,color:"#5a564e"}}>No payments this month.</p>}
            </div>}
          </>}
        </div>
      </Card>;
