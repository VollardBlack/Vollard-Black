...
function GalleryPage(){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,420px) minmax(0,1fr)",gap:24,alignItems:"start"}}>
      ...
      <Card style={{textAlign:"center",padding:18,marginBottom:14}}><div style={{fontSize:14,fontWeight:500,color:diff>0?"#4a9e6b":"#b68b2e"}}>{diff>0?`${s} sale${s!==1?"s":""}/mo saves you ${diff} artwork${diff!==1?"s":""} under management`:`${neededNoSales} collectors x R ${fmt(moPerArt)}/mo = R ${fmt(moPerArt*neededNoSales)}/mo`}</div></Card>
