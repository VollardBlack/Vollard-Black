...
// ─── CATALOGUE ───────────────────────────────────────────────────────
// ─── CATALOGUE PAGE ───────────────────────────────────────────────────
function CataloguePage({ setPage, setCalcWork }) {
  const [tab, setTab] = useState('available');
  ...
  const allWorks = ARTISTS.flatMap(a =>
    a.works.map(w => ({ ...w, artistName: a.name, artistId: a.id, artistImage: a.image }))
  );
  const availableWorks = allWorks.filter(w => w.status === 'available');
  const soldWorks = allWorks.filter(w => w.status === 'sold');

  const artistsWithAvailable = ARTISTS.filter(a => a.works.some(w => w.status === 'available'));
  const artistsWithSold = ARTISTS.filter(a => a.works.some(w => w.status === 'sold'));

  const applyFilters = (works) => works.filter(w => {
    const matchArtist = artistFilter === 'all' || w.artistId === artistFilter;
  ...
  const shownAvailable = applyFilters(availableWorks);
  const shownSold = applyFilters(soldWorks);
  ...
  const artistList = tab === 'available' ? artistsWithAvailable : artistsWithSold;
  ...
  return (
    <div style={{ paddingTop: 72, minHeight: '100vh', background: C.ink }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'start' }}>
              <div style={{ position: 'relative' }}>
                <img
                  src={lightbox.image}
                  alt={lightbox.title}
                  style={{ width: '100%', borderRadius: 6, display: 'block', opacity: lightbox.status === 'sold' ? 0.65 : 1 }}
                />
                {lightbox.status === 'sold' && (
                  <div style={{
                    position: 'absolute', top: 14, right: 14,
                    fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase',
                    color: C.green, background: 'rgba(10,16,32,0.92)', border: `1px solid ${C.green}`,
                    padding: '5px 12px', borderRadius: 3,
                  }}>✓ Sold</div>
                )}
              </div>
              <div>
                <div style={{ fontSize: 10, letterSpacing: '0.35em', textTransform: 'uppercase', color: C.gold, marginBottom: 12 }}>
                  {lightbox.artistName}
                </div>
                <h2 style={{ fontFamily: gF, fontSize: 26, fontWeight: 400, color: C.cream, marginBottom: 20, lineHeight: 1.3 }}>
                  {lightbox.title}
                </h2>
                <div style={{ fontFamily: gF, fontSize: 40, color: C.gold, marginBottom: 24, letterSpacing: '-0.01em' }}>
                  R {fmt(lightbox.price)}
                </div>

                {lightbox.status === 'available' ? (
                  <>
                    {/* Live backing preview */}
                    {[6,12,24].map(months => {
                      const mk = months === 6 ? 'S6' : months === 12 ? 'E12' : 'P24';
                      ...
                      return (
                        <div key={months} style={{
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                            <span style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.fog }}>
                              {months}-Month Term
                            </span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <span style={{ fontSize: 14, fontFamily: gF, color: '#5aaa7a', fontWeight: 400 }}>
                              R {fmt(d.backerNet)}
                            </span>
                          </div>
                        </div>
                    ...
                    <button
                      className="gold-btn"
                      onClick={() => { openCalculator(lightbox); setLightbox(null); }}
                      style={{
                  ...
                  <div style={{ padding: '20px', background: C.goldGlow, border: `1px solid ${C.goldBorder}`, borderRadius: 4, textAlign: 'center' }}>
                    <div style={{ fontFamily: gF, fontSize: 18, color: C.fog, marginBottom: 8 }}>This work has found a home</div>
                    <div style={{ fontSize: 12, color: C.fog, lineHeight: 1.7 }}>
                      Similar works by {lightbox.artistName} may be available — browse the Available catalogue or contact the gallery.
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* Header */}
      <div style={{ padding: '72px 40px 40px', borderBottom: `1px solid ${C.goldBorder}`, background: C.inkMid }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: gF, fontSize: 'clamp(36px, 6vw, 64px)', fontWeight: 300, color: C.cream, letterSpacing: '0.04em', marginBottom: 32 }}>
            The Catalogue
          </h1>

          {/* Tab switcher */}
          <div style={{ display: 'flex', gap: 0, marginBottom: 28, width: 'fit-content', border: `1px solid ${C.goldBorder}`, borderRadius: 6, overflow: 'hidden' }}>
            {[
              ['available', `Available for Backing (${availableWorks.length})`],
              ['sold', `Sold Archive (${soldWorks.length})`],
            ].map(([id, label]) => (
              <button
                key={id}
                onClick={() => { setTab(id); setArtistFilter('all'); }}
                style={{
                  padding: '12px 28px', fontFamily: sF, fontSize: 12, fontWeight: 600,
                  letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', border: 'none',
                  background: tab === id ? `linear-gradient(135deg, ${C.gold}, #a07828)` : C.inkMid,
                  color: tab === id ? '#1a2744' : C.fog,
                }}
              >
                {label}
              </button>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <input
              ...
              style={{
                ...
                background: 'rgba(232,237,245,0.06)', border: `1px solid ${C.goldBorder}`,
                borderRadius: 4, color: C.cream, fontFamily: sF, fontSize: 13, outline: 'none',
              }}
            />
            <select
              ...
              style={{
                padding: '10px 16px', background: C.inkMid, border: `1px solid ${C.goldBorder}`,
                borderRadius: 4, color: C.cream, fontFamily: sF, fontSize: 12, outline: 'none',
      ...
      <div style={{ padding: '48px 40px', maxWidth: 1400, margin: '0 auto' }}>
        {tab === 'available' ? (
          <AvailableCatalogue
            works={shownAvailable}
            allWorks={availableWorks}
            artistFilter={artistFilter}
            setArtistFilter={setArtistFilter}
            onView={setLightbox}
            onBuy={openCalculator}
            mounted={mounted}
          />
        ) : (
          <SoldCatalogue
...
function AvailableCatalogue({ works, allWorks, artistFilter, setArtistFilter, onView, onBuy, mounted }) {
  // Group by artist
  const byArtist = {};
  works.forEach(w => {
    if (!byArtist[w.artistName]) byArtist[w.artistName] = { artistId: w.artistId, artistImage: w.artistImage, works: [] };
    byArtist[w.artistName].works.push(w);
  });

  if (works.length === 0) return (
    <div style={{ textAlign: 'center', padding: '80px 0', color: C.fog }}>
  ...
  return (
    <div>
      {Object.entries(byArtist).map(([artistName, { works: artistWorks }], gi) => (
        <div key={artistName} style={{ marginBottom: 64, animation: mounted ? `fadeUp 0.6s ${gi * 0.05}s ease both` : 'none', opacity: mounted ? 1 : 0 }}>
          {/* Artist section header */}
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            marginBottom: 24, paddingBottom: 16, borderBottom: `1px solid ${C.goldBorder}`,
          }}>
            <div>
              <div style={{ fontSize: 10, letterSpacing: '0.35em', textTransform: 'uppercase', color: C.gold, marginBottom: 6 }}>
                {artistWorks.length} work{artistWorks.length !== 1 ? 's' : ''} available
              </div>
              <h2 style={{ fontFamily: gF, fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 400, color: C.cream, letterSpacing: '0.03em' }}>
                {artistName}
              </h2>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={() => onBuy({ ...artistWorks[0], artistName })}
                style={{
                  padding: '10px 20px', fontFamily: sF, fontSize: 11, fontWeight: 600,
...
function SoldCatalogue({ works, mounted, onView }) {
  if (works.length === 0) return (
    <div style={{ textAlign: 'center', padding: '80px 0', color: C.fog }}>
      <div style={{ fontFamily: gF, fontSize: 48, opacity: 0.2, marginBottom: 16 }}>◆</div>
      <div>No sold works found.</div>
    </div>
  );
  ...
  const byArtist = {};
  works.forEach(w => {
    if (!byArtist[w.artistName]) byArtist[w.artistName] = [];
    byArtist[w.artistName].push(w);
  });
  ...
  return (
    <div>
      <div style={{
        ...
        background: C.goldGlow, border: `1px solid ${C.goldBorder}`, borderRadius: 6,
        display: 'flex', gap: 16, alignItems: 'center',
      }}>
        <div style={{ fontSize: 24, color: C.gold }}>◆</div>
        <div>
          <div style={{ fontFamily: gF, fontSize: 18, color: C.cream, marginBottom: 4 }}>
            These Masterpieces Have Found a Home
          </div>
          <div style={{ fontSize: 12, color: C.fog, lineHeight: 1.7 }}>
            {works.length} sold works across {Object.keys(byArtist).length} artists. This archive demonstrates the market for each artist's work. Looking for something specific? Contact the gallery about commissions.
          </div>
        </div>
      </div>

      {Object.entries(byArtist).map(([artistName, artistWorks], gi) => (
        <div key={artistName} style={{ marginBottom: 56, animation: mounted ? `fadeUp 0.6s ${gi * 0.04}s ease both` : 'none', opacity: mounted ? 1 : 0 }}>
          <div style={{ marginBottom: 20, paddingBottom: 14, borderBottom: `1px solid ${C.goldBorder}` }}>
            <div style={{ fontSize: 10, letterSpacing: '0.35em', textTransform: 'uppercase', color: C.fog, marginBottom: 6 }}>
              {artistWorks.length} sold
            </div>
            <h2 style={{ fontFamily: gF, fontSize: 'clamp(22px, 3vw, 32px)', fontWeight: 400, color: C.cream }}>
              {artistName}
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14 }}>
            {artistWorks.map((work, i) => (
              <WorkCard key={i} work={work} onView={() => onView(work)} showCalcBtn={false} isSold />
            ))}
          </div>
        </div>
      ))}
...
function WorkCard({ work, onView, onCalc, showCalcBtn, isSold }) {
  return (
    <div style={{ borderRadius: 4, overflow: 'hidden', border: `1px solid ${C.goldBorder}`, background: C.inkMid, display: 'flex', flexDirection: 'column' }}>
      {/* Image */}
      <div
        className="art-card"
        onClick={onView}
      ...
      <div style={{ padding: '12px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ fontFamily: gF, fontSize: 13, color: C.cream, lineHeight: 1.3, flex: 1 }}>
          {work.title}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontFamily: gF, fontSize: 20, color: isSold ? C.green : C.gold }}>
            R {fmt(work.price)}
          </div>
          {!isSold && work.price > 0 && (
        ...
        {showCalcBtn && (
          <button
            ...
            style={{
              ...
              background: `linear-gradient(135deg, ${C.gold}, #a07828)`,
              border: 'none', borderRadius: 3, color: '#1a2744',
...
function BackingPage({ preloadWork }) {
  ...
  // Pre-load work from catalogue
  useEffect(() => {
    if (preloadWork) {
      setMode('lookup');
      const artist = ARTISTS.find(a => a.id === preloadWork.artistId);
      if (artist) {
        setSelectedArtist(artist.id);
        const idx = artist.works.findIndex(w => w.title === preloadWork.title && w.price === preloadWork.price);
  ...
  const selectedWorkData = preloadWork || work;

  return (
    <div style={{ paddingTop: 72, minHeight: '100vh', background: C.ink }}>
      <div style={{ padding: '72px 40px 48px', borderBottom: `1px solid ${C.goldBorder}`, background: C.inkMid }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.40em', textTransform: 'uppercase', color: C.gold, marginBottom: 14 }}>Backer Tools</div>
          <h1 style={{ fontFamily: gF, fontSize: 'clamp(36px, 6vw, 60px)', fontWeight: 300, color: C.cream, letterSpacing: '0.04em', marginBottom: 14 }}>
            The Backing Calculator
          </h1>
          <p style={{ fontSize: 14, color: C.fog, lineHeight: 1.8, maxWidth: 580, fontWeight: 300 }}>
            Select an artwork from the gallery or enter any value manually. Model your display fee earnings and sale proceeds across all three backing terms.
          </p>
        </div>
      </div>
      ...
      <div style={{ padding: '56px 40px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 48, alignItems: 'start' }}>
          ...
          <div>
            ...
            {selectedWorkData && selectedWorkData.image && (
              <div style={{
                marginBottom: 28, borderRadius: 6, overflow: 'hidden',
                border: `1px solid ${C.goldBorder}`, background: C.inkMid,
              }}>
                <div style={{ position: 'relative', paddingBottom: '65%', overflow: 'hidden' }}>
                  ...
                  <div style={{
                    position: 'absolute', inset: 0,
                    background: 'linear-gradient(to top, rgba(26,39,68,0.95), transparent 50%)',
                    display: 'flex', alignItems: 'flex-end', padding: 16,
                  }}>
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase', color: C.gold, marginBottom: 4 }}>
                        {selectedWorkData.artistName}
                      </div>
                      <div style={{ fontFamily: gF, fontSize: 15, color: C.cream, lineHeight: 1.3 }}>
            ...
            {/* Mode toggle */}
            <div style={{ display: 'flex', gap: 0, marginBottom: 24, border: `1px solid ${C.goldBorder}`, borderRadius: 4, overflow: 'hidden' }}>
              {[['manual', 'Manual Entry'], ['lookup', 'From Gallery']].map(([id, label]) => (
                <button key={id} onClick={() => setMode(id)} style={{
                  flex: 1, padding: '11px 0', fontFamily: sF,
            ...
            {mode === 'lookup' && (
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.gold, marginBottom: 8 }}>Artist</label>
                <select
                  value={selectedArtist}
                  onChange={e => { setSelectedArtist(e.target.value); setSelectedWork(''); setArtVal(''); }}
                  style={{ width: '100%', padding: '12px 14px', background: C.inkMid, border: `1px solid ${C.goldBorder}`, borderRadius: 4, color: C.cream, fontFamily: sF, fontSize: 13, outline: 'none', cursor: 'pointer', appearance: 'none' }}
                >
                  <option value="">— Select an artist</option>
                  {ARTISTS.filter(a => a.works.some(w => w.status === 'available')).map(a => (
                    <option key={a.id} value={a.id}>{a.name} ({a.works.filter(w=>w.status==='available').length} available)</option>
                  ))}
                </select>
                {artist && (
                  <div style={{ marginTop: 14 }}>
                    <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.gold, marginBottom: 8 }}>Artwork</label>
                    <select
                      value={selectedWork}
                      onChange={e => setSelectedWork(e.target.value)}
                      style={{ width: '100%', padding: '12px 14px', background: C.inkMid, border: `1px solid ${C.goldBorder}`, borderRadius: 4, color: C.cream, fontFamily: sF, fontSize: 12, outline: 'none', cursor: 'pointer', appearance: 'none' }}
                    >
            ...
            {/* Term */}
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.gold, marginBottom: 10 }}>Backing Term</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                {Object.entries(MODELS).map(([key, mod]) => (
                  <button key={key} onClick={() => { setModelKey(key); setMonthsSold(''); }} style={{
                    padding: '14px 6px', borderRadius: 4, cursor: 'pointer', fontFamily: sF, textAlign: 'center',
                    border: modelKey === key ? 'none' : `1px solid ${C.goldBorder}`,
                    background: modelKey === key ? `linear-gradient(135deg, ${C.gold}, #a07828)` : C.inkMid,
                    color: modelKey === key ? '#1a2744' : C.fog,
                  }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 2 }}>{mod.label}</div>
                    <div style={{ fontSize: 9, opacity: 0.8 }}>{mod.term} months</div>
                    {av > 0 && <div style={{ fontSize: 10, marginTop: 3, fontWeight: 600 }}>R {fmt((av * 0.5) / mod.term)}/mo</div>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.gold, marginBottom: 8 }}>Artwork Value (R)</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.gold, marginBottom: 8 }}>
                Expected Sale Price (R) <span style={{ color: C.fog, fontWeight: 300, textTransform: 'none', letterSpacing: 0 }}>— optional</span>
              </label>
              <div style={{ display: 'flex', alignItems: 'center', border: `1px solid ${C.goldBorder}`, borderRadius: 4, overflow: 'hidden', background: C.inkMid }}>
                <span style={{ padding: '0 12px', color: C.fog, borderRight: `1px solid ${C.goldBorder}`, height: 46, display: 'flex', alignItems: 'center' }}>R</span>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <input type="text" inputMode="decimal" value={monthsSold}
                ...
                style={{ width: '100%', padding: '12px 14px', background: C.inkMid, border: `1px solid ${C.goldBorder}`, borderRadius: 4, color: C.cream, fontFamily: sF, fontSize: 15, outline: 'none', textAlign: 'right' }}
              />
            </div>
            ...
            <div style={{ padding: '14px 16px', background: C.goldGlow, border: `1px solid ${C.goldBorder}`, borderRadius: 4, fontSize: 11, color: C.fog, lineHeight: 1.7 }}>
              <strong style={{ color: C.gold }}>FAIS Note:</strong> This is a display license arrangement — not an investment. All figures are illustrative and subject to actual sale outcomes.
            </div>
          </div>
          ...
          <div>
            {!deal ? (
              <div style={{ textAlign: 'center', padding: '80px 40px', border: `1px solid ${C.goldBorder}`, borderRadius: 6, background: C.inkMid }}>
                <div style={{ fontFamily: gF, fontSize: 56, color: C.gold, opacity: 0.15, marginBottom: 20 }}>◆</div>
                <div style={{ fontFamily: gF, fontSize: 22, color: C.fog, fontWeight: 300, marginBottom: 12 }}>Enter an artwork value to begin</div>
                <div style={{ fontSize: 12, color: C.fog, opacity: 0.6 }}>Or choose an artwork from the gallery using the "From Gallery" tab</div>
              </div>
            ) : (
              <>
                {/* 3 hero metric cards */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
                  {[
                    { label: 'Monthly Fee', val: `R ${fmt(deal.monthly)}`, sub: `× ${m.term} months`, color: C.gold },
                    { label: 'Total Collected', val: `R ${fmt(deal.collected)}`, sub: `by month ${mo}`, color: C.goldLight },
                    { label: 'Your Sale Share', val: `R ${fmt(deal.backerNet)}`,
                      sub: deal.backerProfit >= 0 ? `+R ${fmt(deal.backerProfit)} profit` : `−R ${fmt(Math.abs(deal.backerProfit))}`,
                      color: deal.backerProfit >= 0 ? '#5aaa7a' : C.red },
                  ].map(card => (
                    <div key={card.label} style={{ padding: '22px 16px', border: `1px solid ${C.goldBorder}`, borderRadius: 4, background: C.inkMid, textAlign: 'center' }}>
                      <div style={{ fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: C.fog, marginBottom: 10 }}>{card.label}</div>
                      <div style={{ fontFamily: gF, fontSize: 26, color: card.color, marginBottom: 5 }}>{card.val}</div>
                ...
                {/* Deal breakdown */}
                <div style={{ border: `1px solid ${C.goldBorder}`, borderRadius: 4, background: C.inkMid, overflow: 'hidden', marginBottom: 20 }}>
                  <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.goldBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: C.gold }}>
                      Deal Breakdown — {m.label} · Month {mo}
                    </div>
                    {selectedWorkData && (
                      <div style={{ fontSize: 10, color: C.fog }}>{selectedWorkData.artistName}</div>
                    )}
                  </div>
                  <div style={{ padding: '16px 22px' }}>
                    {[
                      ['Artwork value', `R ${fmt(av)}`, false, false, C.gold],
                      ['Backer commission (50%)', `R ${fmt(deal.fee)}`, false, false, C.fog],
                      ['Monthly fee to backer', `R ${fmt(deal.monthly)}`, false, false, C.fog],
                      [`${mo} month${mo > 1 ? 's' : ''} collected`, `R ${fmt(deal.collected)}`, false, false, C.fog],
                      ['Balance owing at sale', `−R ${fmt(deal.balance)}`, deal.balance > 0, false, deal.balance > 0 ? C.red : '#5aaa7a'],
                      'divider',
                      ['Your share at sale', `R ${fmt(deal.backerNet)}`, false, true, '#5aaa7a'],
                      ['Your net profit', `R ${fmt(deal.backerProfit)}`, false, true, deal.backerProfit >= 0 ? '#5aaa7a' : C.red],
                      ['Return on investment', `${fmtDec(deal.roi)}%`, false, true, deal.roi >= 0 ? '#5aaa7a' : C.red],
                    ].map((row, i) => {
                      if (row === 'divider') return <div key={i} style={{ height: 1, background: C.goldBorder, margin: '10px 0' }} />;
                      const [label, val, isNeg, isBold, color] = row;
                      return (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid rgba(201,168,76,0.07)` }}>
                          <span style={{ fontSize: 12, color: C.fog }}>{label}</span>
                          <span style={{ fontFamily: isBold ? gF : sF, fontSize: isBold ? 20 : 13, color, fontWeight: isBold ? 400 : 500 }}>{val}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ border: `1px solid ${C.goldBorder}`, borderRadius: 4, background: C.inkMid, overflow: 'hidden' }}>
                  <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.goldBorder}` }}>
                    <div style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: C.gold }}>
                      All Scenarios — {m.label} ({m.term} months)
                    </div>
                  </div>
                  <div style={{ overflowX: 'auto', maxHeight: 340, overflowY: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                      ...
                      <tbody>
                        {scenarios.map(s => {
                          const isSel = s.month === mo;
                          return (
                            <tr key={s.month} onClick={() => setMonthsSold(String(s.month))}
                              style={{ background: isSel ? C.goldDim : 'transparent', cursor: 'pointer' }}>
                              <td style={{ padding: '9px 14px', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: isSel ? C.gold : C.cream, fontWeight: isSel ? 700 : 400 }}>
                                Mo {s.month}{isSel ? ' ◆' : ''}
                              </td>
                              <td style={{ padding: '9px 14px', textAlign: 'right', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: C.fog }}>R {fmt(s.collected)}</td>
                              <td style={{ padding: '9px 14px', textAlign: 'right', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: s.balance > 0 ? C.red : '#5aaa7a' }}>R {fmt(s.balance)}</td>
                              <td style={{ padding: '9px 14px', textAlign: 'right', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: '#5aaa7a', fontWeight: 600 }}>R {fmt(s.backerNet)}</td>
                              <td style={{ padding: '9px 14px', textAlign: 'right', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: s.backerProfit >= 0 ? '#5aaa7a' : C.red }}>R {fmt(s.backerProfit)}</td>
                              <td style={{ padding: '9px 14px', textAlign: 'right', borderBottom: `1px solid rgba(201,168,76,0.07)`, color: C.gold, fontWeight: 600 }}>{fmtDec(s.roi)}%</td>
                            </tr>
                          );
                        })}
                      </tbody>
...
export default function WinelandsBackers() {
  ...
  return (
    <>
      ...
      {page === 'catalogue' && <CataloguePage setPage={navigateTo} setCalcWork={openCalculator} />}
