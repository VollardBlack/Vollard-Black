...
export default function BidPanel({ artwork, isLive }) {
  const [currentPrice, setCurrentPrice] = useState(artwork.current_price);
  const [whatsapp, setWhatsapp] = useState('');
  const [amount, setAmount] = useState('');
  const [status, setStatus] = useState('idle'); // idle | submitting | success | error
  const [message, setMessage] = useState('');
  const [closed, setClosed] = useState(!isLive);

  const minBid = Number(currentPrice) + Number(artwork.bid_increment);
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch('/api/place-bid', {
      ...
      body: JSON.stringify({ artworkId: artwork.id, whatsapp: wa, amount: Number(amount) }),
    });
    ...
    <div className="bid-panel">
      <div className="price-row">
        <div>
          ...
          <p className="price">R{Number(currentPrice).toLocaleString('en-ZA')}</p>
        </div>
        <div className="timer">
          <p className="label">{closed ? 'Status' : 'Closes in'}</p>
          <p className="timer-value">
            <CountdownTimer endsAt={artwork.ends_at} onComplete={() => setClosed(true)} />
          </p>
        </div>
      </div>

      {closed ? (
        <p className="closed-note">This auction has closed. Bidding is no longer open.</p>
      ) : (
        <form onSubmit={handleSubmit}>
          ...
          <label className="field">
            ...
            <input
              ...
              min={minBid}
              step={artwork.bid_increment}
              required
            />
          </label>

          {message && (
            <p className={status === 'success' ? 'msg-success' : 'msg-error'}>{message}</p>
          )}
